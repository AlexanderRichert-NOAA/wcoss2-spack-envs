...
spack stack create env \
  --name testC-env --template empty --site wcoss2 --compiler oneapi@2024.2.1 \
  --upstream /lfs/h1/emc/nceplibs/noscrub/dev-spack-stack-test/spack-stack-1.9.2rc3/envs/nco-oneapi-2024.2.1/install \
  --dir .

spack env activate testC-env
spack config add 'config:deprecated:true'
spack add ed

spack concretize -f --fresh |& tee testC-env/log.concretize
# Should fail on spack.lock validation:
